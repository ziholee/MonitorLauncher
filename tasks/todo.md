...
## Checklist
- [x] 현재 MainForm의 초기화·새로고침·실행·프로필 저장 경로와 기존 변경을 확인한다
- [x] 기본 모니터 자동 선택을 최초 초기화로 제한하고 실패 후 미선택 상태를 유지한다
- [x] 새로고침 상태 문구를 선택 여부에 맞추고 기존 경고를 유지한다
- [x] 관련 경로 회귀 검사와 가능한 빌드 검증 결과를 기록한다

## 계획 및 완료 조건
- 현재 v2.0.0에 포함된 v1.2.6 모니터 안전성 흐름만 수정한다.
- 유효한 선택은 새로고침 후 복원하고, 무효하거나 비어 있는 선택은 반복 새로고침에도 미선택으로 남긴다.
- 실행/저장 시 무효한 선택을 발견하면 경고와 상태 안내를 유지하며, 사용자가 다시 선택하기 전에는 실행/저장 요청을 만들지 않는다.
- 초기 기본 선택, 재선택, 창 위치 조정, 프로필 실행 경로를 함께 검토한다. .NET/Windows 실행 환경이 없으면 정적 검증과 실행하지 못한 항목을 구분해 기록한다.

## Review
- 원인: 실패 직후 새로고침은 기본 선택을 금지했지만, 수동 새로고침은 기본 인수 `true`를 사용해 비어 있는 선택을 주 모니터로 바꿨다.
- `RefreshMonitorList`의 기본 인수를 `false`로 바꾸고 생성자에서만 `true`를 지정했다. 정상 선택은 스냅샷 매칭으로 복원한다.
- `TryGetSelectedMonitor`는 무효 선택을 먼저 해제하고 목록을 갱신한 뒤 기존 경고와 상태 안내를 표시한다. 이후 화면 재연결이나 반복 새로고침으로 선택이 자동 복원되지 않는다.
- 수동 새로고침 후 미선택이면 상태 표시줄에 사용할 모니터를 다시 선택하라는 안내를 남긴다.
- 변경 전 Python 소스 검사에서 안전한 기본값·초기화의 명시적 기본 선택·미선택 상태 문구 3개 항목의 실패를 확인했다.
- 변경 후 Python 소스 검사 12개 통과: 초기화의 유일한 기본 선택 허용, 정상 선택 복원, fallback 조건, 수동 새로고침/상태, 실패 시 선택 해제 순서/경고/중단, 최신 화면 매칭, 실행/저장 전 검증, 창 위치 조정, 저장된 프로필 실행, 직접 화면 배열 인덱싱 부재. `git diff --check` 통과.
- 제한: 위 검사는 소스 수준 검증이며 런타임 테스트가 아니다. `dotnet build MonitorLauncher.csproj -c Release -p:EnableWindowsTargeting=true`는 `dotnet: command not found`로 실행하지 못했다. Windows에서 모니터 분리 → 실행/저장 경고 → 반복 새로고침 → 명시적 재선택 흐름의 실제 WinForms 검증은 남아 있다.
