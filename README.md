...
5. 같은 이름이 있으면 덮어쓰기 여부 확인 (기본 선택은 "아니요")

**워크스페이스 실행:**
- 저장된 워크스페이스를 선택 후 "실행" 클릭
- 또는 트레이 메뉴의 "워크스페이스 실행"에서 바로 실행
- 복원 중에는 트레이와 폼에서 중복 복원을 차단합니다.
- 프로세스명과 저장된 실행 파일 경로를 확인하고 제목이 같은 창을 우선 선택합니다. 한 번 복원한 창은 같은 복원 작업에서 다시 사용하지 않습니다.
- 제목이 바뀌거나 동일한 창은 정확한 대응이 보장되지 않습니다. 앱 자체가 새 창 생성을 지원하지 않으면 부족한 창은 복원 실패로 표시될 수 있습니다.
- 이동 실패와 저장된 모니터 부재로 인한 주 모니터 대체는 복원 결과에 표시합니다.

**화면 밖 창 복구:**
- 워크스페이스 탭에서 "화면 밖 창 복구" 버튼 클릭
- 모니터 사이 빈 공간을 포함해 제목 영역에 접근하기 어렵거나 크기가 비정상인 창을 주 모니터 중앙으로 이동합니다.
- 파일 탐색기와 Windows Terminal도 저장 대상에 포함하며, 바탕화면과 작업 표시줄은 제외합니다.
- 작은 화면에서는 창 크기를 조절하거나 스크롤하여 하단 기능에 접근할 수 있습니다.

Windows 회귀 검증 절차는 [workspace-regression.md](docs/workspace-regression.md)를 참고하세요.
...
- **v2.0.1**: 창 중복 복원 방지, 화면 밖 창 복구 개선, 복원 결과 안내 및 작은 화면 대응 ([릴리즈 노트](docs/releases/v2.0.1.md))
